...
// tna6
// Get round layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetRoundLayer(const LayerParameter& param) {
  RoundParameter_Engine engine = param.round_param().engine();
  if (engine == RoundParameter_Engine_DEFAULT) {
    engine = RoundParameter_Engine_CAFFE;
  }
  if (engine == RoundParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new RoundLayer<Dtype>(param));
  } else {
    LOG(FATAL) << "Layer " << param.name() << " has unknown engine.";
  }
}

REGISTER_LAYER_CREATOR(Round, GetRoundLayer);
// tna6
